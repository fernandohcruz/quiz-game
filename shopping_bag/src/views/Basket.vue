  ...
  <div class="basket">
    <div class="items">
      ...
      <template v-if="this.productsInBag.length">
        <div class="item" v-for="product in this.productsInBag" v-bind:key="product.id">
          <div class="remove" v-on:click="removeFromBag(product.id)">Remover Produto</div>
          <div class="photo"><img :src="product.image" alt=""></div>
          <div class="description">{{ product.title }}</div>
          <div class="price">
            <span class="quantity-area">
              <button :disabled="product.quantity<=1" v-on:click="decreaseQuantity(product.id)">-</button>
              <span class="quantity">{{ product.quantity }}</span>
              <button v-on:click="increaseQuantity(product.id)">+</button>
            </span>
            <span class="amount">R$ {{ (product.quantity * product.price).toFixed(2) }}</span>
          </div>
        </div>
        <div class="grand-total"> Grand total: R$ {{ totalPrice() }}</div>
      </template>
